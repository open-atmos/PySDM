...
class Simulation:
    def __init__(self, settings, backend=CPU):
        ...
        multiplicities = discretise_multiplicities(
        ...
        products = [
            ...
            PySDM_products.AmbientRelativeHumidity(
                var="RH", name="RH_ice", ice=True, unit="%"
            ),
            ...
            PySDM_products.ParticleConcentration(
                name="n", unit="1/cm**3", radius_range=(-np.inf, np.inf)
            ),
            ...
            PySDM_products.MeanRadius(name="r", radius_range=(-np.inf, np.inf)),
            # PySDM_products.ParticleConcentration(
            #         name='n_c', unit='1/kg',
    ...
    def run(self):
        ...
        for _ in range(self.n_output):
            ...
            print(self.particulator.attributes["multiplicity"].to_ndarray())
            print(self.particulator.attributes["volume"].to_ndarray())

            print(self.particulator.products["water"].get()[0])
            print(self.particulator.products["ice"].get()[0])
            print(self.particulator.products["total"].get()[0])

            print(self.particulator.products["n_s"].get()[0])
            print(self.particulator.products["n_i"].get()[0])
            print(self.particulator.products["n"].get()[0])

            print(self.particulator.products["r_s"].get()[0])
            print(self.particulator.products["r_i"].get()[0])
            print(self.particulator.products["r"].get()[0])
            # print( self.particulator.attributes["radius"].to_ndarray() )
            if any(self.particulator.attributes["volume"].to_ndarray() < 0):
                quit()
