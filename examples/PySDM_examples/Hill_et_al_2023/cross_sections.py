...
def compute_dvol_andsigma_time_series(output, settings, cloud_base):
    # num_conc = (output["nc"] + output["nr"] + output["na"])[cloud_base]
    ...
    mass_bins = (nc * r_bin_volumes[:, np.newaxis]) * 1000  # times density
    Dk = r_bin_centers * 2
    Dvolmass = np.sum(mass_bins * Dk[:, np.newaxis],axis=0) / np.sum(mass_bins,axis=0)
    Nd = np.sum(nc)
    sigma = np.sqrt((1 / (Nd - 1)) * np.sum(nc * (Dk[:, np.newaxis] - Dvolmass)**2,axis=0))
    return D_vol, sigma

def compute_dvol_andsigma_vertical(output, settings, zslice,time):
    t_idx = np.where(output["t"] == time)[0][0]
    # num_conc = (output["nc"] + output["nr"] + output["na"])[zslice, t_idx]
    ...
    r_bin_centers = (r_bin_edges[:-1] + r_bin_edges[1:]) / 2
    r_bin_volumes = settings.formulae.trivia.volume(r_bin_centers) 
    nc = (output["wet spectrum"][zslice,:, t_idx] + output["dry spectrum"][zslice,:, t_idx])
    mass_bins = (nc * r_bin_volumes) * 1000  # times density
    Dk = r_bin_centers * 2
    Dvolmass = np.sum(mass_bins * Dk,axis=1) / np.sum(mass_bins,axis=1) 
    Nd = np.sum(nc, axis=1)
    sigma = np.sqrt((1 / (Nd - 1)) * np.sum(nc * (Dk - Dvolmass[:, np.newaxis])**2, axis=1))
    sigma
    return D_vol, sigma

def compute_LWP_and_nc_time_series(output,settings,z_slice):
    LWP = np.mean(output["LWC"][z_slice], axis=0)*settings.z_max
    # num_conc = (output["nc"]+output["nr"]+output["na"])[z_slice]
