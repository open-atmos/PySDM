...
class Simulation:
    ...
    def __init__(self, settings: Settings, backend: Union[Type[CPU], Type[GPU]] = CPU):
        ...
        self._output_attributes: dict[str, list[Any]] = {a: [None for _ in range(self.num_steps+1)] for a in self.particulator.attributes.keys()}
        self._exec_time: float = -1

    def save(self, index):
        self._output_products["dm/dlnr"][index] = self.particulator.products["dv/dlnr"].get()[
            0]
        self._output_products["dm/dlnr"][index][:] *= self.settings.rho

        self._output_products["terminal_vel"][index] = self.particulator.products["terminal_vel"].get(
        )

        self._output_products["fall_vel"][index] = self.particulator.products["fall_vel"].get(
        )

        for a in self._output_attributes:
            self._output_attributes[a][index] = self.particulator.attributes[a].to_ndarray(
            )
    ...
    def plot_vs_lnr_animation(self, product_name: str, y_scale: float = 1, y_label: Optional[str] = None, num_fixed: int = 4, show=True, speedup=40):
        assert self.done
        ...
        def animate(i):
            update_list = [self._plot_vs_lnr_single(
                i*speedup, product_name, y_scale, plt_colors, set_to=graph), plt.legend()]

            return update_list

        anim = animation.FuncAnimation(fig, animate,
                                       frames=self.num_steps//speedup, interval=50)
    ...
    def plot_sd_velocity_animation(self, show=True, speedup=40):
        ...
        def plot_at_index(i, set_to=None):
            if set_to is not None:
                set_to.set_data(self._output_attributes["radius"][i]*si.metres/si.micrometres, self._output_attributes["fall velocity"][i])
                set_to.set_label(f"t = {i*self.dt}s")
                return set_to

            else:
                return plt.plot(self._output_attributes["radius"][i]*si.metres/si.micrometres, self._output_attributes["fall velocity"][i], "ro")
        ...
        plt.plot(radii_arr*si.metres/si.micrometres, terminal_vel_arr,
        "b-", label="Rogers Yau", alpha=0.5, linewidth=3)
