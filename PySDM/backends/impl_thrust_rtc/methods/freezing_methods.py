    ...
    def __init__(self):
        ...
        self.freeze_time_dependent_body = trtc.For(
            ...
            body=f"""
                ...
                if ({self.formulae.trivia.unfrozen.c_inline(volume="wet_volume[i]")}) {{
                    auto rate = {self.formulae.heterogeneous_ice_nucleation_rate.j_het.c_inline(a_w_ice="a_w_ice[cell[i]]")};
                    auto prob = 1 - exp(
        ...
        self.freeze_singular_body = trtc.For(
            ...
            body=f"""
                ...
                if (
                    {self.formulae.trivia.unfrozen.c_inline(volume="wet_volume[i]")}
                    && relative_humidity[cell[i]] > 1
