...
@numba.njit(float64(float64), **conf.JIT_FLAGS)
def pvs(T):
    # August-Roche-Magnus formula
    return const.ARM_C1 * np.exp((const.ARM_C2 * (T - const.T0)) / (T - const.T0 + const.ARM_C3))


@numba.njit(float64(float64, float64, float64), **conf.JIT_FLAGS)
def _mix(q, dry, wet):
...
@numba.njit(float64(float64, float64), **conf.JIT_FLAGS)
def lambdaK(T, p):
...
@numba.njit([float64(float64, float64, float64, float64, float64, float64)], **conf.JIT_FLAGS)
