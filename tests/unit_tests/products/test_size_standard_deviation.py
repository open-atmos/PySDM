...
#         if n_tot[i] > 0:
#             area_std[i] = np.sqrt(
#                 np.cov(r_sq[:, i][~np.isnan(r_sq[:, i])], fweights=n_act[:, i][~np.isnan(n_act[:, i])]))
#     return area_std
...
class TestSizeStandardDeviation:
    ...
    def test_radius_standard_deviation(backend_class, r, n):
        ...
        particulator = builder.build(
            ...
            products=(RadiusStandardDeviation(name=name, count_activated=True, count_unactivated=True),)
        )
