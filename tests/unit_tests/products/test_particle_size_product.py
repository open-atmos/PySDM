...
from PySDM.products import RadiusStandardDeviation, ActivatedMeanRadius, AreaStandardDeviation, MeanVolumeRadius
...
def radius_std(r, n, mask):
    ...
    std = np.sqrt(np.cov(r_act[~np.isnan(r_act)], fweights=n_act[~np.isnan(n_act)])) if n_tot > 1 else 0
    return std
...
def area_std(r, n, mask):
    ...
    std = np.sqrt(np.cov(r_sq[~np.isnan(r_sq)], fweights=n_act[~np.isnan(n_act)])) if n_tot > 1 else 0
    return std
...
@pytest.mark.parametrize("product_class, validation_fun", (
...
def test_particle_size_product(
    backend_class, r, n, count_unactivated, count_activated,
    ...
    particulator = builder.build(
        ...
        products=(
            product_class(
                name=name, count_activated=count_activated, count_unactivated=count_unactivated
            ),
        ),
    )
