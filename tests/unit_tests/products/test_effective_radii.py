...
def test_effective_radii(backend_class):
    ...
    env = Box(dt=np.nan, dv=np.nan, backend=backend)
    wet_radii = np.asarray([0.01 * si.um, 0.05 * si.um, 0.09 * si.um, 1 * si.um])
    dry_radii = np.asarray([0.009 * si.um] * len(wet_radii))
    kappa = 1.666

    dry_volume = backend.formulae.trivia.volume(radius=dry_radii)
    particulator = Particulator(
        n_sd=len(wet_radii),
        environment=env,
        attributes={
            "water mass": backend.formulae.particle_shape_and_density.radius_to_mass(
                wet_radii
            ),
