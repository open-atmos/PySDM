...
def test_aerosol_init(mass_fractions, plot=False):
    # Arrange
    ...
    vol_fracs = aer.volume_fractions(mass_fractions)

    # Assert
    kappas = {"(NH4)2SO4":0.72, "insoluble":0.0}
    print(sum(kappas[k]*mass_fractions[k] for k in kappas))
    np.testing.assert_approx_equal(
        kappa["Constant"],
        sum(kappas[k]*mass_fractions[k] for k in kappas),
        significant=2
    )
