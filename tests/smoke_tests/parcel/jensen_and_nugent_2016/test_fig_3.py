...
def variables_fixture():
    return notebook_vars(
        file=Path(Jensen_and_Nugent_2016.__file__).parent / "Fig_3.ipynb", plot=PLOT
    )
...
class TestFig3:
    ...
    @pytest.mark.parametrize(
        "drop_id",
        range(
            Jensen_and_Nugent_2016.simulation.N_SD_NON_GCCN // 4,
            Jensen_and_Nugent_2016.simulation.N_SD_NON_GCCN,
        ),
    )
    ...
    #     dradius = np.asarray(variables["output"]["attributes"]["radius"]["drop_id"])
    #     for index, value in enumerate(dradius):
    #         if value.all() > 0:
    #             cloud_base_index = index
    #             break
    #     z0 = variables["settings"].z0
    #     #assert (-300 * si.m < variables["output"]["products"]["z"][cloud_base_index] - z0[-1] < 300 * si.m).all()
    #
    #     assert (
    #             -300 * si.m
    #             < variables["output"]["attributes"]["z"][cloud_base_index] - z0
    #             < 300 * si.m
    #     )
...
# TODO: radius at -300 m, at ascent top, at cloud base (x2: first pass, end of descent)
# TODO: asserts on radii in Fig 3
# TODO: smoke test for radii in Fig 4 (new file)
